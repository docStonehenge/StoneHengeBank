...
module StonehengeBank
  describe Investment do
    context 'a newly created investment' do
      ...
      specify { expect(subject).to have_attributes(interest_rate: 0.06) }
      specify { expect(subject).to have_attributes(period: 1) }
    end

    context 'can be created with a future value instead' do
      subject do
        described_class.new(
          future_value: 100.0, interest_rate: '6% annually', period: '1 year'
        )
      end
